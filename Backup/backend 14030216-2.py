...
def clear_entry(widget):
    for entry in widget.winfo_children():
        if isinstance(entry, Entry):
            ...
            entry.delete(0, END)
            if widget.winfo_name()=="admin_tab":
...
def add_employee_button_clicked(employee_tab):
    ...
    VALUES (
            ...
            data = get_data("Employee", column, f"national_code='{value_list[2]}'")
        messagebox.showinfo("Awareness",
                            f""
                            f"{data[0][0]} {data[0][1]} Added As New Employee \n\n Employee Code: "
                            f"{data[0][2]}\n username: "
                            f"{data[0][3]}")
        clear_entry(employee_tab)
